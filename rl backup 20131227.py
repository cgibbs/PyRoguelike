...
color_lightning     = libtcod.yellow
...
prefixes = {'fast': ['Blitz', 'Whip', 'Zip', 'Flash', 'Spring', 'Swift', 'Sparrow'],
            'strong': ['Brute', 'Crush', 'Slam', 'Punch', 'Grist', 'Paul'],
...
def spell_menu(header):
    if len(player.spellbook) == 0:
        ...
        options = [spell.capitalize() for spell in player.spellbook]

    index = menu(header, options, INVENTORY_WIDTH)

    if index is None or len(spellbook) == 0:
        return None
    return player.spellbook[index]
...
def create_monster(x, y):
    ...
    monster_chances['mimic'] = from_dungeon_level([[15,2],[25,4], [0,10]])
    ...
    monster_chances['lich'] = from_dungeon_level([[10,3], [25, 5], [35, 7], [0,10]])
    ...
    if choice == 'orc':
        ...
        fighter_component = Fighter(hp=54, defense=3, power=3, attack_dice=(2,8),
                                    xp=80, death_function=monster_death)
        ai_component = BasicMonster()
...
def monster_death(monster):
    ...
    tile_effect(monster.x, monster.y, 'blood', 4)
